    ...
    static class Ring {
        ...
        public void setInsets(int i, int i2) {
            ...
            if (this.mRingCenterRadius <= 0.0d || min < 0.0f) {
                f = FloatMath.ceil(this.mStrokeWidth / 2.0f);
            } else {
    ...
    private void setupAnimators() {
        ...
        C01381 r2 = new Animation() {
            public void applyTransformation(float f, Transformation transformation) {
                ...
                if (f <= 0.5f) {
                    ring.setStartTrim(startingStartTrim + ((MaterialProgressDrawable.MAX_PROGRESS_ARC - access$100) * MaterialProgressDrawable.MATERIAL_INTERPOLATOR.getInterpolation(f / 0.5f)));
                }
                if (f > 0.5f) {
                    ring.setEndTrim(startingEndTrim + (MaterialProgressDrawable.MATERIAL_INTERPOLATOR.getInterpolation((f - 0.5f) / 0.5f) * (MaterialProgressDrawable.MAX_PROGRESS_ARC - access$100)));
                }
                ring.setRotation(startingRotation + (0.25f * f));
                MaterialProgressDrawable.this.setRotation((216.0f * f) + ((MaterialProgressDrawable.this.mRotationCount / 5.0f) * MaterialProgressDrawable.FULL_ROTATION));
            }
        };
        r2.setRepeatCount(-1);
        r2.setRepeatMode(1);
