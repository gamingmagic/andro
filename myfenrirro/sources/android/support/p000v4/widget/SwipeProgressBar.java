...
final class SwipeProgressBar {
    ...
    public final void draw(Canvas canvas) {
        ...
        if (this.mRunning || this.mFinishTime > 0) {
            ...
            long j2 = (j - this.mStartTime) / 2000;
            float f = ((float) ((currentAnimationTimeMillis - this.mStartTime) % 2000)) / 20.0f;
            if (!this.mRunning) {
                if (j - this.mFinishTime >= 1000) {
                    this.mFinishTime = 0;
                    return;
                }
                float interpolation = ((float) i) * INTERPOLATOR.getInterpolation((((float) ((j - this.mFinishTime) % 1000)) / 10.0f) / 100.0f);
            ...
            if (f >= 0.0f && f <= 25.0f) {
                drawCircle(canvas, (float) i, (float) i2, this.mColor1, ((25.0f + f) * 2.0f) / 100.0f);
            }
            if (f >= 0.0f && f <= 50.0f) {
                drawCircle(canvas, (float) i, (float) i2, this.mColor2, (2.0f * f) / 100.0f);
            }
            if (f >= 25.0f && f <= 75.0f) {
                drawCircle(canvas, (float) i, (float) i2, this.mColor3, ((f - 25.0f) * 2.0f) / 100.0f);
            }
            if (f >= 50.0f && f <= 100.0f) {
                drawCircle(canvas, (float) i, (float) i2, this.mColor4, ((f - 50.0f) * 2.0f) / 100.0f);
            }
            if (f >= 75.0f && f <= 100.0f) {
                drawCircle(canvas, (float) i, (float) i2, this.mColor1, ((f - 75.0f) * 2.0f) / 100.0f);
            }
